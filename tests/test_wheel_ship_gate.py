...
def test_wheel_builds_cleanly(tmp_path):
    ...
    wheels = list(out.glob("wraith-1.0.0-*.whl"))
    sdists = list(out.glob("wraith-1.0.0.tar.gz"))
...
def test_wheel_installs_into_fresh_venv(tmp_path):
    ...
    assert version_out == "wraith 1.0.0", f"unexpected --version output: {version_out!r}"
...
def test_wheel_version_importable_in_fresh_venv(tmp_path):
    """`import wraith; wraith.__version__` == '1.0.0' inside the fresh venv."""
    ...
    _run([str(py), "-c", "import wraith; assert wraith.__version__ == '1.0.0'"])
